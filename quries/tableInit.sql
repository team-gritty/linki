...
DROP TABLE IF EXISTS collected_channels;
CREATE TABLE `collected_channels` (
	`collected_channel_id` VARCHAR(25) NOT NULL COMMENT '수집된 채널 식별 ID',
	`youtube_channel_id` VARCHAR(255) NOT NULL COMMENT 'YouTube 채널 ID (UC로 시작)',
	`channel_name` VARCHAR(255) NOT NULL COMMENT '채널명',
	`channel_url` VARCHAR(255) NOT NULL COMMENT '채널 URL',
	`channel_description` LONGTEXT NULL COMMENT '채널 설명',
	`channel_thumbnail_url` VARCHAR(500) NULL COMMENT '채널 썸네일 이미지 URL',
	`channel_category` VARCHAR(100) NOT NULL COMMENT '채널 카테고리',
	`channel_country` VARCHAR(100) NULL COMMENT '채널 국가',
	`subscriber_count` BIGINT NULL COMMENT '구독자 수',
	`video_count` INT NULL COMMENT '동영상 수',
	`view_count` BIGINT NULL COMMENT '총 조회수',
	`like_count` BIGINT NULL COMMENT '총 좋아요 수',
	`comment_count` BIGINT NULL COMMENT '총 댓글 수',
	`channel_created_at` DATETIME NULL COMMENT '채널 생성 일자',
	`collected_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '수집 일시',
	`search_keyword` VARCHAR(255) NULL COMMENT '검색에 사용된 키워드'
);
DROP TABLE IF EXISTS subscriber_history;
CREATE TABLE `subscriber_history` (
	`history_id` VARCHAR(25) NOT NULL COMMENT '히스토리 ID',
	`collected_channel_id` VARCHAR(25) NOT NULL COMMENT '수집된 채널 ID',
...
CREATE TABLE `video_statistics` (
	`video_stats_id` VARCHAR(25) NOT NULL COMMENT '통계 ID',
	`collected_channel_id` VARCHAR(25) NOT NULL COMMENT '수집된 채널 ID',
	`video_id` VARCHAR(255) NOT NULL COMMENT 'YouTube 비디오 ID',
	`title` VARCHAR(255) NOT NULL COMMENT '영상 제목',
	`view_count` BIGINT NOT NULL DEFAULT 0 COMMENT '조회수',
	`like_count` BIGINT NOT NULL DEFAULT 0 COMMENT '좋아요 수',
	`comment_count` BIGINT NOT NULL DEFAULT 0 COMMENT '댓글 수',
	`published_at` DATETIME NOT NULL COMMENT '영상 업로드 일시',
	`collected_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '데이터 수집 일시',
	PRIMARY KEY (`video_stats_id`),
	FOREIGN KEY (`collected_channel_id`) REFERENCES `collected_channels`(`collected_channel_id`) ON DELETE CASCADE,
	INDEX `idx_channel_published` (`collected_channel_id`, `published_at`)
) COMMENT '채널별 영상 통계';
ALTER TABLE `refund`
ADD CONSTRAINT `PK_REFUND` PRIMARY KEY (`refund_id`);
ALTER TABLE `influencer`
ADD CONSTRAINT `PK_INFLUENCER` PRIMARY KEY (`influencer_id`);
ALTER TABLE `message`
ADD CONSTRAINT `PK_MESSAGE` PRIMARY KEY (`message_id`);
ALTER TABLE `settlement`
ADD CONSTRAINT `PK_SETTLEMENT` PRIMARY KEY (`settlement_id`);
ALTER TABLE `advertiser_review`
ADD CONSTRAINT `PK_ADVERTISER_REVIEW` PRIMARY KEY (`advertiser_review_id`);
ALTER TABLE `redirect_links`
ADD CONSTRAINT `PK_REDIRECT_LINKS` PRIMARY KEY (`redirect_id`);
ALTER TABLE `user_terms_agreement`
ADD CONSTRAINT `PK_USER_TERMS_AGREEMENT` PRIMARY KEY (`agreement_id`);
ALTER TABLE `payments`
ADD CONSTRAINT `PK_PAYMENTS` PRIMARY KEY (`payment_id`);
ALTER TABLE `subscribe`
ADD CONSTRAINT `PK_SUBSCRIBE` PRIMARY KEY (`subscribe_id`);
ALTER TABLE `channel_stats`
ADD CONSTRAINT `PK_CHANNEL_STATS` PRIMARY KEY (`stats_id`);
ALTER TABLE `notice_view`
ADD CONSTRAINT `PK_NOTICE_VIEW` PRIMARY KEY (`notice_view_id`);
ALTER TABLE `user_terms`
ADD CONSTRAINT `PK_USER_TERMS` PRIMARY KEY (`terms_id`);
ALTER TABLE `campaign`
ADD CONSTRAINT `PK_CAMPAIGN` PRIMARY KEY (`campaign_id`);
ALTER TABLE `signature`
ADD CONSTRAINT `PK_SIGNATURE` PRIMARY KEY (`signature_id`);
ALTER TABLE `user`
ADD CONSTRAINT `PK_USER` PRIMARY KEY (`user_id`);
ALTER TABLE `qna_comment`
ADD CONSTRAINT `PK_QNA_COMMENT` PRIMARY KEY (`qna_comment_id`);
ALTER TABLE `proposal`
ADD CONSTRAINT `PK_PROPOSAL` PRIMARY KEY (`proposal_id`);
ALTER TABLE `jwt_refresh_token`
ADD CONSTRAINT `PK_JWT_REFRESH_TOKEN` PRIMARY KEY (`jwt_id`);
ALTER TABLE `notice`
ADD CONSTRAINT `PK_NOTICE` PRIMARY KEY (`notice_id`);
ALTER TABLE `chat`
ADD CONSTRAINT `PK_CHAT` PRIMARY KEY (`chat_id`);
ALTER TABLE `advertiser`
ADD CONSTRAINT `PK_ADVERTISER` PRIMARY KEY (`advertiser_id`);
ALTER TABLE `channel`
ADD CONSTRAINT `PK_CHANNEL` PRIMARY KEY (`channel_id`);
ALTER TABLE `redirect_click`
ADD CONSTRAINT `PK_REDIRECT_CLICK` PRIMARY KEY (`click_id`);
ALTER TABLE `qna_board`
ADD CONSTRAINT `PK_QNA_BOARD` PRIMARY KEY (`qna_id`);
ALTER TABLE `billing_id`
ADD CONSTRAINT `PK_BILLING_ID` PRIMARY KEY (`billing_id`);
ALTER TABLE `chat_alarm`
ADD CONSTRAINT `PK_CHAT_ALARM` PRIMARY KEY (`chat_alarm_id`);
ALTER TABLE `contract`
ADD CONSTRAINT `PK_CONTRACT` PRIMARY KEY (`contract_id`);
ALTER TABLE `influencer_auth`
ADD CONSTRAINT `PK_INFLUENCER_AUTH` PRIMARY KEY (`inf_auth_id`);
ALTER TABLE `influencer_review`
ADD CONSTRAINT `PK_INFLUENCER_REVIEW` PRIMARY KEY (`influencer_review_id`);
ALTER TABLE `admin`
ADD CONSTRAINT `PK_ADMIN` PRIMARY KEY (`admin_id`);
ALTER TABLE `collected_channels`
ADD CONSTRAINT `PK_COLLECTED_CHANNELS` PRIMARY KEY (`collected_channel_id`);
ALTER TABLE `subscriber_history`
ADD CONSTRAINT `PK_SUBSCRIBER_HISTORY` PRIMARY KEY (`history_id`);
ALTER TABLE `subscriber_history`
ADD CONSTRAINT `FK_SUBSCRIBER_HISTORY_TO_COLLECTED_CHANNELS` FOREIGN KEY (`collected_channel_id`) REFERENCES `collected_channels`(`collected_channel_id`) ON DELETE CASCADE;
CREATE INDEX `IDX_COLLECTED_CHANNELS_YOUTUBE_ID` ON `collected_channels`(`youtube_channel_id`);
CREATE INDEX `IDX_COLLECTED_CHANNELS_CATEGORY` ON `collected_channels`(`channel_category`);
CREATE INDEX `IDX_SUBSCRIBER_HISTORY_COLLECTED_AT` ON `subscriber_history`(`collected_at`);