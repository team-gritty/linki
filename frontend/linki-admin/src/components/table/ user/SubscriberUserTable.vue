...
import { getSubscriberUserListWithKeyset, searchSubscriberUserWithKeyset, exportExcel } from '@/js/user/SubscriberUser.js'
...
const searchConfig = {
  options: [
    { value: 'userId', label: '회원번호', endpoint: '/v1/admin/api/subscriberUsers/search' },
    { value: 'name', label: '이름', endpoint: '/v1/admin/api/subscriberUsers/search' },
    { value: 'email', label: '이메일', endpoint: '/v1/admin/api/subscriberUsers/search' },
    { value: 'phone', label: '연락처', endpoint: '/v1/admin/api/subscriberUsers/search' },
  ],
  placeholder: '검색어를 입력하세요',
  endpoint: '/v1/admin/api/subscriberUsers'
}
...
const loadUsers = async (cursor = null) => {
  try {
    ...
    console.log('🔍 구독자 목록 로드 - cursor:', cursor, 'size:', pageSize)
    
    let response
    if (isSearchMode.value) {
      // 검색 모드
      response = await searchSubscriberUserWithKeyset(
      ...
      // 일반 모드
      response = await getSubscriberUserListWithKeyset(cursor, pageSize)
    }
    ...
    window.alert('구독자 목록을 불러오지 못했습니다.')
  } finally {
...
// ----------------------
onMounted(async () => {
  console.log('🚀 SubscriberUserTable 마운트 시작')
  await loadUsers(null)
})
</script>
