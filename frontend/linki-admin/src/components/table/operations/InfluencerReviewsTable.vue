  ...
  <table class="member-table desktop-view">
    ...
    <tbody>
      ...
      <tr v-else v-for="review in reviews" :key="review.influencerReviewId">
        <td>{{ review.influencerReviewId }}</td>
        <td>{{ review.influencer }}</td>
        <td>{{ review.writer }}</td>
        <td>{{ review.rating }}</td>
  ...
  <div class="mobile-view">
    ...
    <div v-else v-for="review in reviews" :key="review.influencerReviewId" class="member-card">
      <div class="card-header-row">
        <span class="user-id">{{ review.influencerReviewId }}</span>
        <span class="influencer-name">{{ review.influencer }}</span>
        <button type="button"
                class="toggle-visibility"
                :class="(review.visibility === true || review.visibility === 'true') ? 'open' : 'closed'"
                @click="handleToggleVisibility(review)">
  ...
  <div v-if="modalOpen" class="modal-overlay" @click.self="closeModal">
    <div class="modal-content">
      ...
      <div class="modal-fields">
        <div class="modal-field"><span class="modal-label">인플루언서</span><span class="modal-value">{{ selectedReview.influencer }}</span></div>
...
import { getInfluencerReviewsListWithKeyset, searchInfluencerReviewsWithKeyset, exportExcel, toggleReviewVisibility } from '@/js/operations/InfluencerReviews.js'
...
const searchConfig = {
  options: [
    { value: 'influencer', label: '인플루언서', endpoint: '/v1/admin/api/influencerReviews/search' },
    { value: 'writer', label: '작성자', endpoint: '/v1/admin/api/influencerReviews/search' },
    { value: 'contractId', label: '계약 ID', endpoint: '/v1/admin/api/influencerReviews/search' }
  ],
  placeholder: '검색어를 입력하세요',
  endpoint: '/v1/admin/api/influencerReviews'
}
...
const loadReviews = async (cursor = null) => {
  try {
    ...
    console.log('🔍 인플루언서 리뷰 목록 로드 - cursor:', cursor, 'size:', pageSize)
    
    let response
    if (isSearchMode.value) {
      // 검색 모드
      response = await searchInfluencerReviewsWithKeyset(
      ...
      // 일반 모드
      response = await getInfluencerReviewsListWithKeyset(cursor, pageSize)
    }
    ...
    if (response.data) {
      ...
      if (response.data.list) {
        ...
        console.log('📊 인플루언서 리뷰 데이터 로드 완료:', {
    ...
    console.error('인플루언서 리뷰 목록 로드 중 오류:', error)
    window.alert('인플루언서 리뷰 목록을 불러오지 못했습니다.')
  } finally {
...
// ----------------------
// 컴포넌트 마운트 시 인플루언서 리뷰 목록 불러오기
// ----------------------
onMounted(async () => {
  console.log('🚀 InfluencerReviewsTable 마운트 시작')
  await loadReviews(null)
})
...
const handleToggleVisibility = async (review) => {
  try {
    ...
    await toggleReviewVisibility(review.influencerReviewId, newVisibility)
